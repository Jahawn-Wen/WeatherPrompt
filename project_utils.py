...
# -------------- 2) 再加载 X-VLM 的配置 --------------
    ...
    if opt.xvlm_text_config.endswith('.json'):
        with open(opt.xvlm_text_config, 'r') as f:
            config_text = json.load(f)
    else:
        config_text = ruamel.load(open(opt.xvlm_text_config, 'r'))

    # 把命令行里对应的 flag 同步进去
    # —— 同步训练/CLI 时的开关到 X-VLM 配置 —— #
    config_xvlm['use_swin']      = getattr(opt, 'use_swin', False)
    config_xvlm['use_clip_vit']  = getattr(opt, 'use_clip_vit', False)
    config_xvlm['vision_config'] = opt.xvlm_config
    config_xvlm['image_res']     = getattr(opt, 'h', 224)  # 或者 opt.image_res
    config_xvlm['patch_size']    = config_xvlm.get('patch_size', 32)

    config_xvlm['use_roberta']   = getattr(opt, 'use_roberta', False)
    config_xvlm['text_config']   = opt.xvlm_text_config
    # 如果你想本地 path，否则就改成 HF 的名字
    config_xvlm['text_encoder']  = (
        'roberta-base' 
        if getattr(opt, 'use_roberta', False) 
        else '/home/wjh/project/1014/APTM/data/bert-base-uncased'
    )

    # 对比 & 匹配的超参
    config_xvlm['embed_dim']     = getattr(opt, 'embed_dim', 256)
    config_xvlm['temp']          = getattr(opt, 'temp', 0.07)
    config_xvlm['max_tokens']    = getattr(opt, 'max_tokens', 256)

    # 根据需要打开或关闭这些 loss
    config_xvlm['use_mlm_loss']     = getattr(opt, 'use_mlm_loss', True)
    config_xvlm['use_bbox_loss']    = getattr(opt, 'use_bbox_loss', True)
    config_xvlm['use_spatial_loss'] = getattr(opt, 'use_spatial_loss', False)
    ...
    if opt.views == 3:
        if opt.LPN:
            ...
            elif 'use_VIT' in config and opt.use_VIT:
                model = three_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, norm = opt.norm, adain = opt.adain, btnk=opt.btnk, conv_norm=opt.conv_norm, VGG16=opt.use_vgg, Dense=opt.use_dense, VIT=opt.use_VIT)
            else:
                model = three_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, norm = opt.norm, adain = opt.adain, btnk=opt.btnk, conv_norm=opt.conv_norm, VGG16=opt.use_vgg, config_xvlm=config_xvlm, load_text_params=False)
